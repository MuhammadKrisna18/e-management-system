...
from app.infrastructure.external_services.mock_payment_gateway import MockPaymentGatewayService
from app.infrastructure.external_services.mock_refund_payment_service import MockRefundPaymentService
from app.infrastructure.external_services.mock_notification_service import MockNotificationService

# Command Handlers
from app.application.command_handlers.create_event_handler import CreateEventHandler
from app.application.command_handlers.cancel_event_handler import CancelEventHandler
from app.application.command_handlers.create_ticket_category_handler import CreateTicketCategoryHandler
from app.application.command_handlers.disable_ticket_category_handler import DisableTicketCategoryHandler
from app.application.command_handlers.publish_event_handler import PublishEventHandler
from app.application.command_handlers.create_booking_handler import CreateBookingHandler
from app.application.command_handlers.pay_booking_handler import PayBookingHandler
from app.application.command_handlers.request_refund_handler import RequestRefundHandler
from app.application.command_handlers.approve_refund_handler import ApproveRefundHandler
from app.application.command_handlers.reject_refund_handler import RejectRefundHandler
from app.application.command_handlers.mark_refund_payout_handler import MarkRefundPayoutHandler
...
# Query Handlers
from app.application.query_handlers.get_available_events_handler import GetAvailableEventsHandler
from app.application.query_handlers.get_event_details_handler import GetEventDetailsHandler
...
class Container:
    ...
    def __init__(self, db_session=None):
        ...
        self._get_purchased_tickets_handler = None
    ...
    def get_create_booking_handler(self) -> CreateBookingHandler:
        if self._create_booking_handler is None:
            self._create_booking_handler = CreateBookingHandler(
                self.booking_repository, self.event_repository
            )
    ...
    def get_pay_booking_handler(self) -> PayBookingHandler:
        if self._pay_booking_handler is None:
            self._pay_booking_handler = PayBookingHandler(
                self.booking_repository, self.payment_gateway
            )
    ...
    def get_request_refund_handler(self) -> RequestRefundHandler:
        if self._request_refund_handler is None:
            self._request_refund_handler = RequestRefundHandler(
                self.booking_repository, self.refund_repository
            )
    ...
    def get_approve_refund_handler(self) -> ApproveRefundHandler:
        if self._approve_refund_handler is None:
            self._approve_refund_handler = ApproveRefundHandler(
                self.refund_repository, self.booking_repository
            )
    ...
    def get_reject_refund_handler(self) -> RejectRefundHandler:
        if self._reject_refund_handler is None:
            self._reject_refund_handler = RejectRefundHandler(self.refund_repository)
    ...
    def get_mark_refund_payout_handler(self) -> MarkRefundPayoutHandler:
        if self._mark_refund_payout_handler is None:
            self._mark_refund_payout_handler = MarkRefundPayoutHandler(
                self.refund_repository, self.refund_payment_service
            )
    ...
    def get_expire_booking_handler(self) -> ExpireBookingHandler:
        if self._expire_booking_handler is None:
            self._expire_booking_handler = ExpireBookingHandler(self.booking_repository)
    ...
    def get_check_in_ticket_handler(self) -> CheckInTicketHandler:
        if self._check_in_ticket_handler is None:
            self._check_in_ticket_handler = CheckInTicketHandler(
                self.ticket_repository, self.event_repository
            )
    ...
    def get_available_events_handler(self) -> GetAvailableEventsHandler:
        if self._get_available_events_handler is None:
            self._get_available_events_handler = GetAvailableEventsHandler(self.event_repository)
    ...
    def get_event_details_handler(self) -> GetEventDetailsHandler:
        if self._get_event_details_handler is None:
            self._get_event_details_handler = GetEventDetailsHandler(self.event_repository)
    ...
    def get_refund_details_handler(self) -> GetRefundDetailsQueryHandler:
        if self._get_refund_details_handler is None:
            self._get_refund_details_handler = GetRefundDetailsQueryHandler(self.refund_repository)
    ...
    def get_customer_refunds_handler(self) -> GetCustomerRefundsQueryHandler:
        if self._get_customer_refunds_handler is None:
            self._get_customer_refunds_handler = GetCustomerRefundsQueryHandler(self.refund_repository)
    ...
    def get_approved_refunds_handler(self) -> GetApprovedRefundsQueryHandler:
        if self._get_approved_refunds_handler is None:
            self._get_approved_refunds_handler = GetApprovedRefundsQueryHandler(self.refund_repository)
    ...
    def get_event_sales_report_handler(self) -> GetEventSalesReportQueryHandler:
        if self._get_event_sales_report_handler is None:
            self._get_event_sales_report_handler = GetEventSalesReportQueryHandler(
                self.event_repository, self.booking_repository, self.ticket_repository
            )
    ...
    def get_purchased_tickets_handler(self) -> GetPurchasedTicketsQueryHandler:
        if self._get_purchased_tickets_handler is None:
            self._get_purchased_tickets_handler = GetPurchasedTicketsQueryHandler(
                self.booking_repository, self.ticket_repository, self.event_repository
            )
